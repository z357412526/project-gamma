...
\abstract{We report analyses intended to explore the functional magnetic
  resonance imaging (fMRI) data collected in studies conducted by Repovs et 
  al.,
...
classical linear modeling methods and machine learning, were undertaken in 
order
...
waveform, for each voxel. A high value of the correlation coefficient means 
that
...
calculated as 
\begin{equation}
r = \frac{\Sigma  ( X-\bar{X}) ( Y-\bar{Y})}{\sqrt{ \Sigma ( X-\bar{X} ) ^2 
\Sigma ( Y-\bar{Y})  ^2 }}
\end{equation}

The two methods we are testing here are differentiated by two types of 
reference
...
%\caption{An example of removing first principal components to reduce noise}
%\end{figure}
